...
def saliency(model, batch_fn, target_fn, device,
    ...
    # Use Aurora's built-in backbone-only bfloat16 autocast instead of wrapping
    # everything. A global bfloat16 autocast covering the decoder causes NaN
    # gradients in the backward pass through the Perceiver deaggregate_levels
    # cross-attention used for atmospheric variables (surface vars are unaffected
    # because they bypass the Perceiver entirely). Aurora's internal autocast
    # restricts bfloat16 to the backbone, leaving the decoder in float32 where
    # the Perceiver backward is numerically stable.
