...
def cluster_features(feature_map, n_clusters=None, distance_threshold=0.08):
    """Cluster spatial tokens by cosine similarity.

    Provide exactly one of ``n_clusters`` or ``distance_threshold``.  A fixed
    cluster count makes the occlusion-forward budget explicit; a distance
    threshold retains the original data-dependent behaviour.

    Args:
        feature_map:        np.ndarray (feat_H, feat_W, D)
        n_clusters:         fixed cluster budget, or None to use
                            ``distance_threshold``. Clamped to the spatial token
                            count.
        distance_threshold: AgglomerativeClustering threshold on
                            [1 - cosine_sim], or None to use ``n_clusters``.
...
def aggregate_and_upsample(
    ...
    Args:
        ...
        smooth_sigma:     Gaussian smoothing sigma (tuple or scalar); None = no smoothing
